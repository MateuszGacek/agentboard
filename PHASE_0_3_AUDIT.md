...
`apps/api/src/modules/auth/security.ts` hashed passwords with Node `crypto.scrypt`, while `docs/03_ARCHITECTURE.md` requires Argon2id or bcrypt.
...
- `PHASE_AUDIT.md` is stale and still describes the repository before Phase 2. Keep `PHASE_0_3_AUDIT.md` as the current audit reference or update the old file in a later documentation cleanup.
