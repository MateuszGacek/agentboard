...
Kanban passed a local DB-backed runtime smoke with Docker Compose PostgreSQL,
...
- Workspace: `/Users/mg/Documents/WebDev/vibeApp/kanban`
- Branch: `main`
...
| `docker compose --env-file .env.local -f docker-compose.yml -f /tmp/kanban-postgres-port.yml up -d --force-recreate postgres` | PASS            | Published Postgres on `127.0.0.1:5432` for host-side pnpm commands. |
...
| `pnpm --filter @kanban/api dev`                                                                                               | PASS            | API listened on `http://localhost:3000/api`.                        |
| `pnpm --filter @kanban/web dev`                                                                                               | PASS            | Vite served `http://localhost:5173`.                                |
| `curl http://localhost:3000/api/health`                                                                                       | PASS            | Returned `{"ok":true,"service":"kanban-api",...}`.                  |
...
- `https://kanban.matgac.pl/api/health` fails normal TLS verification because
  the server presents `TRAEFIK DEFAULT CERT`.
- `curl -k https://kanban.matgac.pl/api/health` returns `HTTP 503` with
  `no available server`.
...
issuance/proxy routing are configured for `kanban.matgac.pl`. After that, rerun
